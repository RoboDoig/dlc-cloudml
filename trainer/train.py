...
def train(flags):
    ...
    dataset = create_dataset(cfg, flags.data_dir)
    ...
    # Restore variables from disk. (+ adjustment for cloud ml)
    cfg.init_weights = cfg.init_weights.replace('../', '')
    cfg.init_weights = os.path.join(flags.weights_dir, cfg.init_weights)
